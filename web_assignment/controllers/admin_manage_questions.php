...
include 'views/admin/viewadminedit.php';
?>
...
<script>
    ...
    function updateUrlParams(params) {
        ...
        // If no page is passed, default to page 1
    ...
    function loadProducts(category = 0, pagenum = 1, search = "", order = "") {
        $.ajax({
            url: "models/fetchDB_edit.php",
            type: "GET",
            data: { category: category, page: pagenum, search: search, order: order },
    ...
    $(document).ready(function () {
        ...
        // Initially load products and update URL if needed.
        updateUrlParams({ category: currentCategory, search: currentSearch, order: currentOrder });
        ...
        $("#searchInput").on("input", function () {
            ...
            updateUrlParams({ search: currentSearch, category: currentCategory, order: currentOrder });
            loadProducts(currentCategory, 1, currentSearch, currentOrder); // Giữ nguyên danh mục
        });

        $("#sortSelect").change(function () {
            currentOrder = $(this).val();
            updateUrlParams({ order: currentOrder, search: currentSearch, category: currentCategory });
    ...
    document.addEventListener("DOMContentLoaded", function () {
        // When a tab is shown update the URL using History API
            const tabButtons = document.querySelectorAll('#editTabs button');
            ...
            // On page load, check if activeTab parameter exists and activate that tab
            const urlParams = new URLSearchParams(window.location.search);
            const activeTabParam = urlParams.get('activeTab');
            if (activeTabParam) {
                const tabTrigger = document.querySelector(`#editTabs button[data-bs-target="${activeTabParam}"]`);
                if (tabTrigger) {
...
<script>
    ...
    $(document).ready(function () {
        ...
        // Update all previews using TinyMCE content if available,
        // otherwise fallback to the standard input value.
        ...
        $("#editQuestionForm").submit(function (e) {
            ...
            if (
                ...
                alert("Key does not match any of the options A, B, C, or D!.");
